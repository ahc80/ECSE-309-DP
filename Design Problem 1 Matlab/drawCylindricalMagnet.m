...
%     light
    ...
    surf(x(:,1:round(end/2)),y(:,1:round(end/2)),z(:,1:round(end/2)),'Tag','upper');
    cc = findobj('Tag','upper');
    if e3'*m == 1
        set(cc,'facecolor',colors(2,:),'edgecolor',colors(2,:));
    else            % flip the colors if the magnets is flipped
        set(cc,'facecolor',colors(1,:),'edgecolor',colors(1,:));
    end

    hold on
    surf(x(:,round(end/2)+1:end),y(:,round(end/2)+1:end),z(:,round(end/2)+1:end),'Tag','lower');
    cc = findobj('Tag','lower');
%     shading flat
    if e3'*m == 1
        set(cc,'facecolor',colors(1,:),'edgecolor',colors(1,:));
    else            % flip the colors if the magnets is flipped
    ...
    if strcmp(texture,'normal')    % NORMAL TEXTURE
        ...
        set(cc,'facecolor',colors(1,:),'edgecolor',colors(1,:));
        hold on
        surf(x(:,round(end/2):end),y(:,round(end/2):end),z(:,round(end/2):end),'Tag','lower');
        cc = findobj('Tag','lower');
        set(cc,'facecolor',colors(2,:),'edgecolor',colors(2,:));
        light
    end
