...
if (type == 2)                                     
    ...
    xsn    = gridS(1);              % sensors # along x dimension
    ysn    = gridS(2);              % sensors # along y dimension
    zsn    = gridS(3);              % sensors # along z dimension
...
% build the z-coordinate of the sensor grid
    if (mod(zsn,2) == 0)
        zsens = -(((zsn/2)-.5)*ds):ds:(((zsn/2)-.5)*ds);
    else
        zsens = -((zsn-1)*ds/2):ds:((zsn-1)*ds/2);
    end

    h = 1;
    for i = 1:length(xsens)
        for j = 1:length(ysens)
            for k = 1:length(zsens)
                SensorPosMatrix(h,:) = [xsens(i) ysens(j) zsens(k)];
                h = h+1;
            end
        end
    end
