...
Rscale = 0.007*norm(X); % default radius of the origin of the reference frame
h = 0.05*norm(X);       % default height of the cone
w = 0.08*norm(X);      % default width of the cone
...
plot3(xaxis(:,1),xaxis(:,2),xaxis(:,3),'color','r','linewidth',1)
hold on
plot3(yaxis(:,1),yaxis(:,2),yaxis(:,3),'color','g','linewidth',1)
hold on
plot3(zaxis(:,1),zaxis(:,2),zaxis(:,3),'color','b','linewidth',1)
hold on
