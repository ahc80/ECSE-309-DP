...
%% parse arguments
validModes = {'none','auto','scale','avg','norm','log'};
checkString = @(x) any(validatestring(x,validModes));
...
addOptional(p, 'scaleMode', 'auto', checkString);        %'none' -> vectors are not scaled at all (the length = the magnitude). 'scale' -> vectors lengths multiplied by scale. 'norm -> all vectors use the same length as in "scale". 'log' -> all vectors are log10 transformed then scaled
addParameter(p, 'LineWidth', 0.5, @isscalar);
addParameter(p, 'MaxHeadSize', 0.5, @isscalar);
...
%% vector length and color scaling
I = sqrt(u.^2 + v.^2 + w.^2); %magnitude of each vector

I(I==0) = 1;

un = u ./ I;    %u unit vector
vn = v ./ I;    %u unit vector
wn = w ./ I;    %u unit vector
...
if verLessThan('matlab','9.6')
    ...
    if Imax == inf; Imax = 1e10^9;
    end
    if Imin == -inf; Imin = -1e10^9;
    end    
    ...
    if strcmp('none',mode)
        lenscale = 1;                       %no scaling
    elseif strcmp('auto',mode)
        lenscale = scale*dr_mean/Imax; 
    elseif strcmp('scale',mode)        
        lenscale = scale;                   %linear scaling 
    elseif strcmp('norm',mode)
        lenscale = scale./I;                %set all to the same length = scale
    elseif strcmp('avg',mode)
        lenscale = scale*dr_mean*(I/Imax);  %set all range of 0:scale
    elseif strcmp('log',mode)
        lenscale = 1./log10(I);             %log scaling to prevent blow ups
        lenscale = lenscale - min(lenscale);
        lenscale = scale .* lenscale;
    else
        disp('Select a quiverC3D scale mode: "none" "auto" "scale" "norm" "avg" "log"')
        lenscale = 1;                       %no scaling
    end
...
I = log10(I);   %convert magnitudes to log domain to spread colors better
...
if Imin < 0
    Ic = I + abs(Imin); %move all numbers above 0
    Imax = Imax + abs(Imin);
    Imin = 0;
else
    Ic = I;
